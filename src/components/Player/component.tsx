...
export default function Player(props: propsType) {
  ...
  const [playStatus, setPlayStatus] = useState(status || CoreState.PAUSED);
  const [loading, setLoading] = useState(false);
  const [loopMode, setLoopMode] = useState(CoreState.STOP);
  ...
  const onpause = () => {
    setPlayStatus(CoreState.PAUSED);
  }
  const onplay = () => {
    if (playingPlaylist.length === 0) return;
    setPlayStatus(CoreState.PLAYING);
  }
  const onnext = () => {
  ...
  useEffect(() => {
    ...
    if (playStatus === CoreState.PLAYING) {
      newSong.play(); 
    }
  }, [newSong])

  useEffect(() => { 
    if (playStatus === CoreState.PAUSED) {
      if (!newSong) return
      newSong.pause();
    } else if (playStatus === CoreState.PLAYING) {
      if (!newSong) return
      newSong.play();
    }
  }, [playStatus])
  ...
  return (
    <div className="lr-player container mx-auto flex items-center p-4 w-full h-full">
      <div className="join p-2">
        ...
        {(playStatus === CoreState.PAUSED || playStatus === CoreState.STOP) &&
